...
extension Notification.Name {
    static let argoAppSettingsDidChange = Notification.Name("argo.appSettingsDidChange")
}
...
final class WorkspaceStore: ObservableObject {
    ...
    private var allCommandPaletteItems: [CommandPaletteItem] {
        ...
        if ArgoFeatureFlags.showsRemoteSessionCreationUI {
            items.append(
                CommandPaletteItem(
        ...
        items.append(
            contentsOf: ArgoFeatureRegistry.shared.commandPaletteItems(
                context: ArgoExtensionContext(
    ...
    func loadIfNeeded() async {
        ...
        NotificationCenter.default.post(name: .argoAppSettingsDidChange, object: appSettings)
    ...
    func updateQuickCommands(
        ...
        settings.quickCommandPresets = QuickCommandCatalog.normalizedCommands(
            ...
            reservedShortcuts: ArgoKeyboardShortcuts.effectiveShortcuts(in: settings)
        )
    ...
    func insertQuickCommand(_ preset: QuickCommandPreset) {
        ...
        switch argoQuickCommandDispatch(for: preset) {
    ...
    func rememberSSHPresetSelection(selectedPresetID: UUID?) {
        let updatedSelection = argoRememberedSSHPresetSelection(
    ...
    func rememberAgentPresetSelection(selectedPresetID: UUID?) {
        let updatedSelection = argoRememberedAgentPresetSelection(
        ...
        case .statusMessage(let text, let tone, let deliverSystemNotification, let workspaceID, let worktreePath):
            ...
            if deliverSystemNotification && appSettings.dynamicIslandEnabled {
                ...
                if deliverSystemNotification && appSettings.systemNotificationsEnabled {
                    WorkspaceNotificationCenter.shared.deliver(title: "Argo", body: text, workspaceID: workspaceID, worktreePath: worktreePath)
                }
            }
        }
    }
    ...
    private func openArgoWebsite() {
        guard let url = URL(string: "https://argo.dev") else { return }
    ...
    private func submitArgoFeedback() {
        guard let url = URL(string: "https://github.com/krystal1110/Argo/issues/new") else { return }
        NSWorkspace.shared.open(url)
...
func argoRememberedAgentPresetSelection(
...
func argoRememberedSSHPresetSelection(
