...
enum ArgoGhosttyConfigManager {
    ...
    static func managedConfigContents(settings: AppSettings) -> String {
        ...
        let configuredOpacity = settings.terminalBackgroundOpacity
    ...
    private static func readThemeFileContents(named name: String) -> String? {
        guard let path = ArgoGhosttyThemeCatalog.findThemeFile(named: name),
    ...
    static func managedConfigFileURL(fileManager: FileManager = .default) -> URL {
        argoStateDirectoryURL(fileManager: fileManager)
            ...
            .appendingPathComponent("argo-managed.config")
    }
