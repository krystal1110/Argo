...
@MainActor
final class ArgoGhosttyController: ManagedTerminalSessionSurfaceController {
    ...
    private let terminalView: ArgoGhosttySurfaceView
    ...
    init(launchConfiguration: TerminalLaunchConfiguration) {
        ...
        self.terminalView = ArgoGhosttySurfaceView()
    ...
    func beginSearch(initialText: String?) {
        if let initialText, !initialText.isEmpty {
            _ = terminalView.performBindingAction(argoGhosttySearchBindingAction(for: initialText))
            return
        }
    ...
    func updateSearch(_ text: String) {
        _ = terminalView.performBindingAction(argoGhosttySearchBindingAction(for: text))
    }

    func searchNext() {
        _ = terminalView.performBindingAction(argoGhosttySearchNavigationBindingAction(.next))
    }

    func searchPrevious() {
        _ = terminalView.performBindingAction(argoGhosttySearchNavigationBindingAction(.previous))
    }
    ...
    func startManagedSessionIfNeeded() {
        terminalView.ensureSurface(runtime: ArgoGhosttyRuntime.shared, launchConfiguration: launchConfiguration)
        terminalView.syncSurfaceMetrics()
    }

    func restartManagedSession() {
        terminalView.recreateSurface(runtime: ArgoGhosttyRuntime.shared, launchConfiguration: launchConfiguration)
        terminalView.syncSurfaceMetrics()
    }

    func terminateManagedSession() {
        ArgoGhosttySecureInputManager.shared.release(controller: self)
    ...
    func handleGhosttyAction(_ action: ghostty_action_s, on surface: ghostty_surface_t) -> Bool {
        ...
        case GHOSTTY_ACTION_DESKTOP_NOTIFICATION:
            ...
            if islandEnabled {
                ...
                ArgoGhosttyNotificationCenter.shared.deliver(title: title, body: body)
            }
            return true
        ...
        case GHOSTTY_ACTION_SCROLLBAR:
            terminalView.scrollbarState = ArgoGhosttyScrollbarState(action.action.scrollbar)
            return true
        ...
        case GHOSTTY_ACTION_SECURE_INPUT:
            ArgoGhosttySecureInputManager.shared.apply(action.action.secure_input, controller: self)
            return true
    ...
    func handleSurfaceClose(processAlive: Bool) {
        guard argoGhosttyShouldReportProcessExitForSurfaceClose(processAlive: processAlive) else { return }
    ...
    func handleCommandFinished(_ action: ghostty_action_command_finished_s) {
        ...
        _ = argoGhosttyShouldReportProcessExitForCommandFinished(action)
    }
    ...
    func confirmClipboardWrite(text: String, location: ghostty_clipboard_e) {
        confirmClipboardWrite(
            items: [ArgoGhosttyClipboardPayload(mimeType: "text/plain;charset=utf-8", text: text)],
            location: location
        )
    }

    func confirmClipboardWrite(items: [ArgoGhosttyClipboardPayload], location: ghostty_clipboard_e) {
        let alert = NSAlert()
        ...
        guard alert.runModal() == .alertFirstButtonReturn,
              let pasteboard = argoGhosttyPasteboard(for: location) else {
            return
        }
...
func argoGhosttyShouldReportProcessExitForSurfaceClose(processAlive: Bool) -> Bool {
    !processAlive
}

func argoGhosttyShouldReportProcessExitForCommandFinished(
    _: ghostty_action_command_finished_s
) -> Bool {
    false
}

func argoGhosttyShouldRefreshSurface(
    after previous: ArgoGhosttySurfaceMetricsSignature?,
    next: ArgoGhosttySurfaceMetricsSignature
) -> Bool {
    previous != next
}

private struct ArgoGhosttyScrollbarState {
    let total: UInt64
    let offset: UInt64
    let length: UInt64
...
struct ArgoGhosttySurfaceMetricsSignature: Equatable {
    let width: Int
    let height: Int
    let scale: Double
...
@MainActor
private final class ArgoGhosttySurfaceView: NSView {
    weak var controller: ArgoGhosttyController?
    ...
    var scrollbarState: ArgoGhosttyScrollbarState? {
        didSet { controller?.notifyStatusChange() }
    }
    ...
    private var lastMetricsSignature: ArgoGhosttySurfaceMetricsSignature?
    ...
    func ensureSurface(runtime: ArgoGhosttyRuntime, launchConfiguration: TerminalLaunchConfiguration) {
        guard surface == nil else { return }
        createSurface(runtime: runtime, launchConfiguration: launchConfiguration)
    }

    func recreateSurface(runtime: ArgoGhosttyRuntime, launchConfiguration: TerminalLaunchConfiguration) {
        destroySurface()
        createSurface(runtime: runtime, launchConfiguration: launchConfiguration)
    }
    ...
    private func reconcileFirstResponderForWorkspaceFocus() {
        ...
        guard argoGhosttyShouldReclaimFirstResponder(
    ...
    func destroySurface() {
        ...
        if let surfaceUserdataToken {
            ArgoGhosttyControllerRegistry.shared.unregister(surfaceUserdataToken)
    ...
    func syncSurfaceMetrics() {
        ...
        let nextSignature = ArgoGhosttySurfaceMetricsSignature(
            width: width,
            height: height,
            scale: scale,
            displayID: displayID
    ...
    func applyBackingLayerBackground() {
        wantsLayer = true
        let opacity = ArgoGhosttyRuntime.shared.terminalBackgroundOpacity
        if opacity < 1 {
    ...
    override func keyDown(with event: NSEvent) {
        ...
        if let deleteSequence = argoGhosttyOptionDeleteEscapeSequence(keyCode: event.keyCode, modifierFlags: event.modifierFlags) {
            sendSSHWordNavigation(deleteSequence)
            return
        }

        if let escapeSequence = argoGhosttySSHWordNavigationEscapeSequence(
        ...
        if textModifiers.isEmpty {
            ...
            if ArgoGhosttyTextInputRouting.shouldSyncPreeditAfterTextInterpretation(
            ...
            if !ArgoGhosttyTextInputRouting.shouldDispatchRawKeyFallbackAfterTextInterpretation(
            ...
            let composing = ArgoGhosttyTextInputRouting.shouldMarkRawKeyEventAsComposing(
                hadMarkedTextBeforeInterpretation: hadMarkedTextBeforeInterpretation,
                hasMarkedTextAfterInterpretation: hasMarkedTextAfterInterpretation
            )
            sendRawKeyEvent(
    ...
    override func performKeyEquivalent(with event: NSEvent) -> Bool {
        ...
        if argoGhosttyShouldAttemptMenuKeyEquivalent(
            bindingFlags: flags,
            modifierFlags: event.modifierFlags,
    ...
    override func flagsChanged(with event: NSEvent) {
        ...
        guard let action = argoGhosttyModifierAction(
            keyCode: event.keyCode,
            modifierFlags: event.modifierFlags
        ) else {
            return
    ...
    override func doCommand(by selector: Selector) {
        ...
        switch ArgoGhosttyTextInputCommandAction.resolve(selector: selector, hasMarkedText: hasMarkedText()) {
        case .scrollToTop:
    ...
    @IBAction func find(_ sender: Any?) {
        if let selected = currentSelectionText(), !selected.isEmpty {
            _ = performBindingAction(argoGhosttySearchBindingAction(for: selected))
        } else {
    ...
    @IBAction func findNext(_ sender: Any?) {
        _ = performBindingAction(argoGhosttySearchNavigationBindingAction(.next))
    }

    @IBAction func findPrevious(_ sender: Any?) {
        _ = performBindingAction(argoGhosttySearchNavigationBindingAction(.previous))
    }
    ...
    @IBAction override func performTextFinderAction(_ sender: Any?) {
        guard let action = argoTextFinderAction(for: sender) else {
    ...
    func readSelection(from pboard: NSPasteboard) -> Bool {
        guard let string = pboard.argoGhosttyBestString else { return false }
        sendText(string)
        return true
    }

    private func createSurface(runtime: ArgoGhosttyRuntime, launchConfiguration: TerminalLaunchConfiguration) {
        guard let controller else { return }
        backendConfiguration = launchConfiguration.backendConfiguration

        let surfaceUserdataToken = ArgoGhosttyControllerRegistry.shared.register(controller)
    ...
    private func deleteBackwardInMarkedText() {
        ...
        var state = ArgoGhosttyMarkedTextState(text: markedText.string, selectedRange: markedSelectionRange)
    ...
    private func draggedText(from pasteboard: NSPasteboard) -> String? {
        ...
        return argoTerminalDropText(fileURLs: fileURLs, plainText: pasteboard.string(forType: .string))
    }
...
extension ArgoGhosttySurfaceView: @preconcurrency NSServicesMenuRequestor {}

extension ArgoGhosttySurfaceView: NSMenuItemValidation {}

extension ArgoGhosttySurfaceView: @preconcurrency NSTextInputClient {
    ...
    func selectedRange() -> NSRange {
        ...
        return ArgoGhosttyMarkedTextState.clamp(markedSelectionRange, textLength: markedText.length)
    }
    ...
    func insertText(_ string: Any, replacementRange: NSRange) {
        ...
        if ArgoGhosttyTextInputRouting.shouldTreatInsertedTextAsMarkedTextDuringDeletion(
...
private final class ArgoGhosttyIMEDebugLogger {
    static let shared = ArgoGhosttyIMEDebugLogger(environment: ProcessInfo.processInfo.environment)

    private let enabled: Bool
    private let processID = ProcessInfo.processInfo.processIdentifier

    init(environment: [String: String]) {
        self.enabled = argoGhosttyShouldEnableIMEDebugLogging(environment: environment)
    }
...
@MainActor
final class ArgoGhosttyNotificationCenter {
    static let shared = ArgoGhosttyNotificationCenter()
    ...
    func deliver(title: String, body: String?) {
        ...
        let request = UNNotificationRequest(
            identifier: "com.argo.ghostty.\(UUID().uuidString)",
            content: content,
            trigger: nil
        )
        center.add(request)
...
@MainActor
private final class ArgoGhosttySecureInputManager {
    static let shared = ArgoGhosttySecureInputManager()

    private var activeControllers: Set<ObjectIdentifier> = []

    func apply(_ action: ghostty_action_secure_input_e, controller: ArgoGhosttyController) {
    ...
    func release(controller: ArgoGhosttyController) {
...
@MainActor
final class ArgoGhosttyControllerRegistry {
    static let shared = ArgoGhosttyControllerRegistry()

    private final class WeakBox {
        weak var controller: ArgoGhosttyController?

        init(controller: ArgoGhosttyController) {
    ...
    func register(_ controller: ArgoGhosttyController) -> UnsafeMutableRawPointer {
    ...
    func controller(for address: UInt?) -> ArgoGhosttyController? {
        guard let address else { return nil }
        return controllers[address]?.controller
    }

    func liveControllers() -> [ArgoGhosttyController] {
