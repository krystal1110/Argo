...
enum TerminalSurfaceFactory {
    ...
    static func make(
        ...
        if argoIsRunningTests() {
            return ArgoTestManagedTerminalSurfaceController(launchConfiguration: launchConfiguration)
        }
        return ArgoGhosttyController(launchConfiguration: launchConfiguration)
    }
}
...
func argoTextFinderAction(for sender: Any?) -> NSTextFinder.Action? {
    guard let menuItem = sender as? NSMenuItem else { return nil }
    return NSTextFinder.Action(rawValue: menuItem.tag)
}

enum ArgoGhosttySearchNavigation: String {
    case previous
    case next
}

func argoGhosttySearchBindingAction(for query: String) -> String {
    "search:\(query)"
}

func argoGhosttySearchNavigationBindingAction(_ direction: ArgoGhosttySearchNavigation) -> String {
    "navigate_search:\(direction.rawValue)"
}

func argoTerminalDropText(fileURLs: [URL], plainText: String?) -> String? {
