...
final class ApplicationMenuController: NSObject {
    ...
    private let feedbackURL = URL(string: "https://github.com/krystal1110/Argo/issues/new")!
    private let repositoryURL = URL(string: "https://github.com/krystal1110/Argo")!

    var activeWorkspaceStoreProvider: (() -> WorkspaceStore?)?

    private var shortcutItemsByAction: [ArgoShortcutAction: [NSMenuItem]] = [:]
    ...
    func applySettings(_ settings: AppSettings) {
        for action in ArgoShortcutAction.allCases {
            ...
            if action == .selectTabByNumber {
                let shortcut = ArgoKeyboardShortcuts.effectiveShortcut(for: action, in: settings)
                for item in items {
    ...
    private func addShortcutItem(
        title: String,
        shortcutAction: ArgoShortcutAction,
        to menu: NSMenu,
        target: AnyObject
    ) -> NSMenuItem {
