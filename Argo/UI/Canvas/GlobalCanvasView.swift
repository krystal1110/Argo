...
struct GlobalCanvasView: View {
    ...
    private var headerPanel: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            .padding(.vertical, 6)
            .background(ArgoTheme.sidebarSearchBackground, in: RoundedRectangle(cornerRadius: 6, style: .continuous))
        ...
        .background(ArgoTheme.chromeBackground.opacity(0.96), in: RoundedRectangle(cornerRadius: 10, style: .continuous))
        .overlay(
    ...
    private var canvasToolbar: some View {
        ...
        .labelStyle(.iconOnly)
        .padding(12)
        .background(ArgoTheme.chromeBackground.opacity(0.96), in: Capsule())
        .overlay(Capsule().stroke(ArgoTheme.border, lineWidth: 1))
        ...
        Button(action: action) {
            ...
            .background(
                RoundedRectangle(cornerRadius: 999, style: .continuous)
                    .fill(isSelected ? ArgoTheme.panelRaised : ArgoTheme.subtleFill)
            )
            .overlay(
                RoundedRectangle(cornerRadius: 999, style: .continuous)
                    .stroke(isSelected ? ArgoTheme.accent.opacity(0.32) : ArgoTheme.border, lineWidth: 1)
            )
        }
        .buttonStyle(.plain)
...
private struct GlobalCanvasCardView: View {
    ...
    private var titleBar: some View {
        HStack(spacing: 6) {
            ...
            Menu {
                ...
                Image(systemName: "ellipsis.circle")
                    ...
                    .foregroundStyle(card.isSelected ? .white : ArgoTheme.secondaryText)
            }
            .menuStyle(.borderlessButton)
            .fixedSize()
        ...
        .padding(.vertical, 4)
        .background(ArgoTheme.chromeBackground.opacity(0.94))
        .gesture(
    ...
    private var minimizedSummary: some View {
        HStack(spacing: 10) {
            WorkspaceCanvasBadge(text: localizedFormat("canvas.card.activeCountFormat", card.activeSessionCount), tint: ArgoTheme.success)
    ...
    private var content: some View {
        if let node = card.tab.layout {
            ...
            .padding(10)
            .background(ArgoTheme.paneBackground)
    ...
    private var cardBackground: Color {
        card.isSelected
            ? ArgoTheme.panelRaised
            : ArgoTheme.panelBackground
    }

    private var cardBorder: Color {
        (layout.colorGroup == .none ? (card.isSelected ? ArgoTheme.accent : ArgoTheme.border) : accentTint.opacity(card.isSelected ? 0.65 : 0.36))
    }
    ...
    var body: some View {
        ...
        .overlay(
            RoundedRectangle(cornerRadius: 5, style: .continuous)
                .stroke(isFocused ? ArgoTheme.accent.opacity(0.4) : ArgoTheme.border, lineWidth: 1)
        )
    }
}
