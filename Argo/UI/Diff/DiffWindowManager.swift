...
final class DiffWindowManager: NSObject, NSWindowDelegate {
    ...
    func show(worktreePath: String?, branchName: String, emptyStateMessage: String) {
        ...
        newWindow.identifier = NSUserInterfaceItemIdentifier("argo.diff")
        newWindow.styleMask = [.titled, .closable, .miniaturizable, .resizable]
        newWindow.tabbingMode = .preferred
        newWindow.tabbingIdentifier = ArgoDesktopApplication.sharedWindowTabbingIdentifier
        newWindow.toolbarStyle = .unifiedCompact
        newWindow.isReleasedWhenClosed = false
        newWindow.minSize = NSSize(width: 760, height: 520)
        newWindow.setFrameAutosaveName("ArgoDiffWindow")

        let hasSavedFrame = UserDefaults.standard.string(forKey: "NSWindow Frame ArgoDiffWindow") != nil
        if !hasSavedFrame {
    ...
    private func adjustZoom(by delta: Double) {
        let current = UserDefaults.standard.double(forKey: "argo.diff.zoom")
    ...
    private func setZoom(_ level: Double) {
        UserDefaults.standard.set(level, forKey: "argo.diff.zoom")
        applyZoom(level)
    }
