...
struct HistoryWindowContentView: View {
    ...
    @AppStorage("argo.history.viewStyle") private var diffStyleRaw = HistoryDiffPresentationStyle.split.rawValue
    ...
    private var commitListPanel: some View {
        VStack(spacing: 0) {
            ...
            .padding(.vertical, 7)
            .background(ArgoTheme.chromeBackground)
            ...
            if let rangeStart = state.rangeStartCommitID {
                ...
                .padding(.horizontal, 10)
                .padding(.vertical, 5)
                .background(ArgoTheme.accent.opacity(0.15))
                .overlay(alignment: .bottom) {
                    Rectangle().fill(ArgoTheme.border).frame(height: 1)
                }
            }
    ...
    private var blamePanel: some View {
        VStack(spacing: 0) {
            if case .blame(let path, let commit) = state.viewMode {
                ...
                .padding(.vertical, 10)
                .background(ArgoTheme.chromeBackground.opacity(0.96))
                .overlay(alignment: .bottom) {
                    Rectangle().fill(ArgoTheme.border).frame(height: 1)
                }
            }
            ...
            if state.isLoadingBlame {
                ...
                .background(ArgoTheme.canvasBackground)
            }
        }
        .frame(maxWidth: .infinity, maxHeight: .infinity)
    }
            ...
            HStack(spacing: 0) {
                ...
                if commit.isMergeCommit {
                    Text("merge")
                        ...
                        .padding(.vertical, 2)
                        .background(ArgoTheme.accent.opacity(0.12), in: Capsule())
                }
            }
...
private struct HistoryCommitDetailHeader: View {
    ...
    var body: some View {
        ...
        .padding(.horizontal, 10)
        .padding(.vertical, 8)
        .background(ArgoTheme.chromeBackground.opacity(0.5))
        .overlay(alignment: .bottom) {
            Rectangle().fill(ArgoTheme.border).frame(height: 1)
        }
    }
...
private struct HistoryBlameLineRow: View {
    ...
    var body: some View {
        ...
        .padding(.vertical, 1)
        .background(line.id % 2 == 0 ? ArgoTheme.canvasBackground : ArgoTheme.canvasBackground.opacity(0.7))
    }
}
