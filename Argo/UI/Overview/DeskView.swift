...
struct DeskView: View {
    ...
    private var accentColor: Color {
        workspace.supportsRepositoryFeatures ? ArgoTheme.accent : ArgoTheme.localAccent
    }
    ...
    var body: some View {
        ...
        .background(ArgoTheme.panelBackground, in: RoundedRectangle(cornerRadius: 6, style: .continuous))
        .overlay(
            RoundedRectangle(cornerRadius: 6, style: .continuous)
                .strokeBorder(isHovering ? accentColor.opacity(0.5) : ArgoTheme.border, lineWidth: 1)
        )
    ...
    private var deskHeader: some View {
        HStack(spacing: 8) {
            ...
            if workspace.supportsRepositoryFeatures {
                HStack(spacing: 3) {
                    Circle()
                        .fill(workspace.hasUncommittedChanges ? ArgoTheme.warning : ArgoTheme.success)
    ...
    private var emptyDesk: some View {
        VStack(spacing: 6) {
            Image(systemName: "desktopcomputer")
                .font(.system(size: 20))
                .foregroundStyle(ArgoTheme.mutedText.opacity(0.4))
            Text(localized("desk.empty.noActiveSessions"))
                .font(.system(size: 10, weight: .medium))
                .foregroundStyle(ArgoTheme.mutedText.opacity(0.5))
        }
        ...
        VStack(spacing: 4) {
            ...
            ZStack {
                RoundedRectangle(cornerRadius: 2, style: .continuous)
                    .fill(isActive ? ArgoTheme.paneBackground : Color.black.opacity(0.3))
                ...
                // Power LED
                Circle()
                    .fill(isActive ? ArgoTheme.success : ArgoTheme.danger.opacity(0.4))
