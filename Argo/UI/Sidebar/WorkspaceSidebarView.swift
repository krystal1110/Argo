...
private final class WorkspaceSidebarCoordinator: NSObject, NSOutlineViewDataSource, NSOutlineViewDelegate {
    private static let workspaceDragType = NSPasteboard.PasteboardType("com.argo.workspace.ids")
    private static let groupDragType = NSPasteboard.PasteboardType("com.argo.group.id")
        ...
        VStack(spacing: 0) {
            HStack(spacing: 6 * uiScale) {
                ...
                Text(group.name.uppercased())
                    ...
                    .tracking(0.5)
                    .foregroundStyle(isSelected ? ArgoTheme.tertiaryText : ArgoTheme.mutedText)
                    .lineLimit(1)
                    .truncationMode(.tail)
                ...
                Text("\(childCount)")
                    ...
                    .foregroundStyle(ArgoTheme.mutedText.opacity(0.6))
            }
            ...
            Rectangle()
                .fill(ArgoTheme.border.opacity(0.5))
                .frame(height: 0.5)
                .padding(.leading, 2 * uiScale)
                .padding(.trailing, 8 * uiScale)
        }
        .background(
            ArgoTheme.subtleFill.opacity(isHovering ? 0.8 : 0),
    ...
    var body: some View {
        ZStack {
            ...
            Circle()
                .fill(activityColor)
                .frame(width: badgeSize, height: badgeSize)
                .overlay(Circle().stroke(ArgoTheme.sidebarBackground, lineWidth: 1))
