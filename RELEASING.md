...
- `TAP_PROJECT_PATH=owner/homebrew-tap ./deploy.sh` (defaults to `<github-owner>/homebrew-tap`)
...
When `TAP_PROJECT_PATH` and `TAP_REMOTE_URL` are omitted, Homebrew cask publishing uses the GitHub owner from `GITHUB_REPOSITORY` and updates `owner/homebrew-tap`. For `krystal1110/Argo`, the generated install command is `brew install --cask krystal1110/tap/argo`.
