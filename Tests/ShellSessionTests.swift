...
final class ShellSessionTests: XCTestCase {
    func testGhosttyShellIntegrationInjectsZshEnvironmentFromBundledResources() {
        ...
        let prepared = ArgoGhosttyShellIntegration.prepare(
            ...
            resourcePaths: ArgoGhosttyResourcePaths(resourceRootURL: resourcesRoot)
        )
    ...
    func testGhosttyShellIntegrationPreservesExistingShellFeaturesWhileAppendingSSHEnv() {
        let prepared = ArgoGhosttyShellIntegration.prepare(
            ...
            resourcePaths: ArgoGhosttyResourcePaths(
    ...
    func testGhosttyBootstrapPublishesBundledResourcesDirectory() {
        let environment = ArgoGhosttyBootstrap.processEnvironment(
            resourcePaths: ArgoGhosttyResourcePaths(
                ghosttyResourcesDirectory: "/tmp/argo-ghostty",
                terminfoDirectory: "/tmp/argo-terminfo"
            )
        )

        XCTAssertEqual(environment["GHOSTTY_RESOURCES_DIR"], "/tmp/argo-ghostty")
    }
    ...
    func testAugmentedExecutablePathPrependsCommonUserAndHomebrewDirectories() {
        XCTAssertEqual(
            argoAugmentedExecutablePath("/usr/bin:/bin", homeDirectory: "/Users/tester"),
            [
    ...
    func testSSHBackendForcesRemoteTTYForInteractiveSessions() {
        ...
        let launchConfiguration = configuration.makeLaunchConfiguration(
            preferredWorkingDirectory: "/tmp/argo-ssh",
            baseEnvironment: [:]
        )
    ...
    func testStartIfNeededOnlyAutoStartsIdleSession() async {
        await MainActor.run {
            ...
            let session = ShellSession(
                snapshot: PaneSnapshot.makeDefault(cwd: "/tmp/argo-shell-session"),
    ...
    func testRestartTransitionsExitedSessionBackToRunning() async {
        await MainActor.run {
            let surface = FakeManagedTerminalSurfaceController()
            let session = ShellSession(
                snapshot: PaneSnapshot.makeDefault(cwd: "/tmp/argo-shell-session-restart"),
                surfaceController: surface
            )
    ...
    func testIsRunningTracksForegroundCommandStateInsteadOfShellLifetime() async {
        await MainActor.run {
            let surface = FakeManagedTerminalSurfaceController()
            let session = ShellSession(
                snapshot: PaneSnapshot.makeDefault(cwd: "/tmp/argo-shell-session-command-state"),
                surfaceController: surface
            )
    ...
    func testRestartReapsPreviousLaunchConfiguration() async {
        await MainActor.run {
            ...
            let session = ShellSession(
                snapshot: PaneSnapshot.makeDefault(cwd: "/tmp/argo-shell-session-reap-restart"),
                surfaceController: surface,
    ...
    func testTerminateReapsCurrentLaunchConfiguration() async {
        await MainActor.run {
            ...
            var reapedConfigurations: [TerminalLaunchConfiguration] = []
            let session = ShellSession(
                snapshot: PaneSnapshot.makeDefault(cwd: "/tmp/argo-shell-session-reap-terminate"),
                surfaceController: surface,
                processReaper: { reapedConfigurations.append($0) }
            )
    ...
    func testSendShellCommandUsesCarriageReturnSubmission() async {
        await MainActor.run {
            let surface = FakeManagedTerminalSurfaceController()
            let session = ShellSession(
                snapshot: PaneSnapshot.makeDefault(cwd: "/tmp/argo-shell-session-send-command"),
                surfaceController: surface
            )
    ...
    func testInsertTextDoesNotAppendReturn() async {
        await MainActor.run {
            let surface = FakeManagedTerminalSurfaceController()
            let session = ShellSession(
                snapshot: PaneSnapshot.makeDefault(cwd: "/tmp/argo-shell-session-insert-text"),
                surfaceController: surface
            )
    ...
    func testSnapshotPromotesLocalTmuxSessionToRestorableLaunch() async {
        await MainActor.run {
            let surface = FakeManagedTerminalSurfaceController()
            let session = ShellSession(
                snapshot: PaneSnapshot.makeDefault(cwd: "/tmp/argo-shell-session-tmux"),
                surfaceController: surface
            )
    ...
    func testProcessReaperTerminatesShellProcessGroupAndLoginProcess() throws {
        ...
        let launchConfiguration = TerminalLaunchConfiguration(
            ...
            environment: [ArgoTerminalManagedProcessReaper.metadataPathEnvironmentKey: metadataPath],
            command: TerminalCommandDefinition(
                executablePath: "/bin/zsh",
                arguments: ["-l"],
                displayName: "zsh"
        ...
        let processControl = ArgoTerminalManagedProcessControl(
        ...
        ArgoTerminalManagedProcessReaper.reap(
            launchConfiguration,
            fileManager: .default,
