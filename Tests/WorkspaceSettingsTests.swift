...
final class WorkspaceSettingsTests: XCTestCase {
    ...
    func testSeededRandomRepositoryIconIsStableForSameRepositoryName() {
        let first = SidebarItemIcon.randomRepository(preferredSeed: "Argo", avoiding: [])
        let second = SidebarItemIcon.randomRepository(preferredSeed: "Argo", avoiding: [])

        XCTAssertEqual(first, second)
    }

    func testSeededRandomRepositoryIconFallsBackWhenPreferredChoiceAlreadyUsed() {
        let preferred = SidebarItemIcon.randomRepository(preferredSeed: "Argo", avoiding: [])
        let avoided = SidebarItemIcon.randomRepository(preferredSeed: "Argo", avoiding: [preferred])
