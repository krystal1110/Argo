...
Argo currently uses macOS system notifications (`UNUserNotificationCenter`) for events like worktree/terminal task completion. This has two limitations:
...
1. Find the `ArgoDesktopApplication` window containing the target workspace
...
- Current notification code: `Argo/Support/WorkspaceNotificationCenter.swift`
- Current settings: `Argo/Domain/AppSettings.swift`
