...
**Files:**
- Modify: `Argo/Services/Git/GitRepositoryService.swift:43-49`
...
Run: `xcodebuild -project Argo.xcodeproj -scheme Argo -destination 'platform=macOS' test -only-testing:ArgoTests/RemoteGitInspectionTests/testParseRemoteGitInspectionWithWorktrees 2>&1 | tail -20`
...
Run: `xcodebuild -project Argo.xcodeproj -scheme Argo -destination 'platform=macOS' test -only-testing:ArgoTests/RemoteGitInspectionTests 2>&1 | tail -20`
Expected: ALL PASS
...
**Files:**
- Modify: `Argo/Domain/WorkspaceRuntime.swift:153-155`
...
Run: `xcodebuild -project Argo.xcodeproj -scheme Argo -destination 'platform=macOS' test -only-testing:ArgoTests/RemoteWorkspaceModelTests/testRemoteServerSupportsRepositoryFeatures 2>&1 | tail -20`
...
Run: `xcodebuild -project Argo.xcodeproj -scheme Argo -destination 'platform=macOS' test -only-testing:ArgoTests/RemoteWorkspaceModelTests 2>&1 | tail -20`
Expected: PASS

**Step 5: Commit**

```bash
git add Argo/Domain/WorkspaceRuntime.swift Tests/RemoteWorkspaceModelTests.swift
...
**Files:**
- Modify: `Argo/App/WorkspaceStore.swift:65` (refresh interval)
- Modify: `Argo/App/WorkspaceStore.swift:914-916` (icon selection)
- Modify: `Argo/App/WorkspaceStore.swift:3042-3060` (refresh logic)
...
Run: `xcodebuild -project Argo.xcodeproj -scheme Argo -destination 'platform=macOS' test 2>&1 | tail -30`
Expected: ALL PASS
...
**Files:**
- Modify: `Argo/Services/Git/GitRepositoryService.swift` (add `createRemoteWorktree` method)
- Modify: `Argo/App/WorkspaceStore.swift` (update `createWorktree` to handle remote)
- Modify: `Argo/App/WorkspaceStore.swift` (update `presentCreateWorktree` to handle remote)
...
Update `CreateWorktreeSheetRequest` in `Argo/Support/UIState.swift:77-82`:
...
In `Argo/UI/Sheets/CreateWorktreeSheet.swift`, when `request.isRemote` is true:
...
```bash
git add Argo/Services/Git/GitRepositoryService.swift Argo/App/WorkspaceStore.swift Argo/Support/UIState.swift Argo/UI/Sheets/CreateWorktreeSheet.swift
...
**Files:**
- Modify: `Argo/Services/Git/GitRepositoryService.swift` (add `removeRemoteWorktree`)
- Modify: `Argo/App/WorkspaceStore.swift` (update `confirmPendingWorktreeRemoval`)
...
```bash
git add Argo/Services/Git/GitRepositoryService.swift Argo/App/WorkspaceStore.swift
...
**Files:**
- Modify: `Argo/UI/Sidebar/WorkspaceSidebarView.swift` (verify worktree rendering)
- Modify: `Argo/App/WorkspaceStore.swift` (verify worktree switching for remote)
...
**Files:**
- Modify: `Argo/Services/Git/GitRepositoryService.swift` (extend SSH command for per-worktree status)
- Modify: `Argo/App/WorkspaceStore.swift` (populate `worktreeStatuses`)
...
```bash
git add Argo/Services/Git/GitRepositoryService.swift Argo/App/WorkspaceStore.swift Tests/RemoteGitInspectionTests.swift
...
```bash
xcodebuild -project Argo.xcodeproj -scheme Argo -destination 'platform=macOS' test 2>&1 | tail -40
```
...
```bash
xcodebuild -project Argo.xcodeproj -scheme Argo -destination 'platform=macOS' build 2>&1 | tail -20
```
